...
export default function AdminSettings() {
  ...
  const toggleItems = [
    { l: t('admin.maintenanceMode'),   icon: 'Wrench'      as const, v: maintenanceMode,   set: setMaintenanceMode   },
    { l: t('admin.registrationsOpen'), icon: 'CheckCircle' as const, v: registrationsOpen, set: setRegistrationsOpen },
    { l: t('admin.aiRecognition'),     icon: 'Zap'         as const, v: aiEnabled,          set: setAiEnabled         },
  ];
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: C.cream }}>
      ...
      <View style={{ flexDirection: 'row', alignItems: 'center', paddingHorizontal: 16, paddingVertical: 14, backgroundColor: '#1A237E' }}>
        ...
        <Text style={{ color: '#fff', fontSize: 15, fontWeight: '700', flex: 1 }}>{t('admin.globalSettings')}</Text>
      ...
      <ScrollView style={{ flex: 1 }} contentContainerStyle={{ padding: 16, paddingBottom: 40 }} showsVerticalScrollIndicator={false}>
        ...
        {maintenanceMode && (
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8, padding: 12, borderRadius: 14, backgroundColor: C.errorSoft, borderWidth: 1, borderColor: '#C6282840', marginBottom: 16 }}>
            ...
            <Text style={{ flex: 1, color: '#C62828', fontSize: 13, fontWeight: '600' }}>{t('admin.maintenanceBanner')}</Text>
          </View>
        )}

        {/* Access */}
        <Text style={{ fontSize: 11, fontWeight: '600', color: C.inkMute, textTransform: 'uppercase', letterSpacing: 0.8, marginBottom: 8 }}>{t('admin.accessAvailability')}</Text>
        ...
        {/* Finance */}
        <Text style={{ fontSize: 11, fontWeight: '600', color: C.inkMute, textTransform: 'uppercase', letterSpacing: 0.8, marginBottom: 8 }}>{t('admin.finance')}</Text>
        <View style={{ borderRadius: 18, backgroundColor: C.surface, borderWidth: 1, borderColor: C.border, padding: 16, marginBottom: 16 }}>
          <Text style={{ fontSize: 14, color: C.ink, marginBottom: 8 }}>{t('admin.kflCommissionOrders')}</Text>
        ...
        {/* App info */}
        <Text style={{ fontSize: 11, fontWeight: '600', color: C.inkMute, textTransform: 'uppercase', letterSpacing: 0.8, marginBottom: 8 }}>{t('admin.appInfo')}</Text>
        <View style={{ borderRadius: 18, backgroundColor: C.surface, borderWidth: 1, borderColor: C.border, overflow: 'hidden' }}>
          {[
            { l: t('admin.version'), v: '1.0.0 (build 42)' },
            { l: 'API',              v: 'v2.1.0' },
