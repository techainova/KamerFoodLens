      ...
      >
        ...
        <View style={{ paddingHorizontal: 16, paddingTop: 18, gap: 9 }}>
          {[
            ...
            { key: 'journal', icon: 'BookOpen' as const, title: t('profile.myJournal', 'Mon journal alimentaire'), subtitle: t('profile.myJournalSubtitle', 'Suivez les plats que vous mangez'), color: C.gold, bg: C.goldSoft, count: 0, onPress: () => navigation.navigate('FoodJournal') },
            { key: 'recipes', icon: 'ChefHat' as const, title: t('profile.allRecipes', 'Toutes les recettes'), subtitle: t('profile.allRecipesSubtitle', 'Parcourir les recettes camerounaises'), color: C.navy, bg: C.navySoft, count: 0, onPress: () => navigation.navigate('AllRecipes') },
            { key: 'forum', icon: 'MessageSquare' as const, title: t('profile.forum', 'Forum communautaire'), subtitle: t('profile.forumSubtitle', 'Échangez avec la communauté KFL'), color: C.primary, bg: 'rgba(232,89,26,0.1)', count: 0, onPress: () => navigation.navigate('Forum') },
          ].map((row) => (
            <TouchableOpacity
              key={row.key}
              onPress={row.onPress}
