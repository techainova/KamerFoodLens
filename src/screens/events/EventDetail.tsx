  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: '#FFFAF5' }}>
      ...
      <ScrollView style={{ flex: 1 }} contentContainerStyle={{ paddingBottom: 120 }} showsVerticalScrollIndicator={false}>
        ...
        {/* Info chips */}
        <View style={{ backgroundColor: '#fff', paddingHorizontal: 16, paddingVertical: 16, borderBottomWidth: 1, borderColor: '#F5F0EB', gap: 10 }}>
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 10 }}>
            ...
            <Text style={{ fontSize: 14, color: '#2C1810', fontWeight: '600' }}>{event.date} · {event.time}</Text>
          </View>
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 10 }}>
            <View style={{ width: 32, height: 32, borderRadius: 16, backgroundColor: '#E8591A15', alignItems: 'center', justifyContent: 'center' }}>
              <Icon name="Clock" size={15} color="#E8591A" />
            </View>
            <Text style={{ fontSize: 14, color: '#6D4C41' }}>Durée : {event.duration}</Text>
          </View>
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 10 }}>
            <View style={{ width: 32, height: 32, borderRadius: 16, backgroundColor: '#E8591A15', alignItems: 'center', justifyContent: 'center' }}>
              <Icon name="MapPin" size={15} color="#E8591A" />
            </View>
            <Text style={{ fontSize: 14, color: '#6D4C41' }} numberOfLines={1}>{event.location}</Text>
          </View>
        </View>

        {/* Seats progress */}
        <View style={{ marginHorizontal: 16, marginTop: 16, backgroundColor: '#fff', borderRadius: 16, padding: 14, borderWidth: 1, borderColor: '#E5E0D8', ...SHADOW_SM }}>
          <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginBottom: 8 }}>
            <Text style={{ fontSize: 13, fontWeight: '600', color: '#2C1810' }}>Places disponibles</Text>
          ...
          <Text style={{ fontSize: 11, color: '#8C8278', marginTop: 6 }}>{event.attendees}/{event.totalSeats} inscrits</Text>
        </View>

        {/* Description */}
        <View style={{ paddingHorizontal: 16, paddingTop: 20 }}>
          <Text style={{ fontSize: 16, fontFamily: 'PlayfairDisplay-Bold', color: '#2C1810', marginBottom: 10 }}>Description</Text>
          <Text style={{ fontSize: 14, color: '#6D4C41', lineHeight: 22 }}>{event.description}</Text>
        </View>

        {/* Programme */}
        <View style={{ paddingHorizontal: 16, paddingTop: 20 }}>
          <Text style={{ fontSize: 16, fontFamily: 'PlayfairDisplay-Bold', color: '#2C1810', marginBottom: 12 }}>Programme</Text>
          <View style={{ gap: 0 }}>
            {event.program.map((item: { time: string; label: string }, i: number) => (
              <View key={i} style={{ flexDirection: 'row', gap: 12 }}>
                ...
                <View style={{ paddingBottom: 16 }}>
                  ...
                  <Text style={{ fontSize: 14, color: '#2C1810' }}>{item.label}</Text>
                </View>
              </View>
            ))}
          </View>
        ...
        {/* Organizer */}
        <View style={{ marginHorizontal: 16, marginTop: 20, backgroundColor: '#fff', borderRadius: 16, padding: 16, borderWidth: 1, borderColor: '#E5E0D8', flexDirection: 'row', gap: 12, alignItems: 'flex-start', ...SHADOW_SM }}>
          ...
          <View style={{ flex: 1 }}>
            <Text style={{ fontSize: 14, fontWeight: '700', color: '#2C1810', marginBottom: 2 }}>{event.organizer}</Text>
            <Text style={{ fontSize: 12, color: '#6D4C41', lineHeight: 18 }}>{event.organizerBio}</Text>
          </View>
        </View>
      </ScrollView>
      ...
      <View style={{ position: 'absolute', bottom: 0, left: 0, right: 0, padding: 16, backgroundColor: '#fff', borderTopWidth: 1, borderColor: '#E5E0D8' }}>
        {event.price > 0 && (
          <Text style={{ fontSize: 18, fontWeight: '700', color: '#2C1810', textAlign: 'center', marginBottom: 10 }}>
