  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: '#FFFAF5' }}>
      ...
      <View style={{ height: 56, paddingHorizontal: 16, flexDirection: 'row', alignItems: 'center', gap: 10, backgroundColor: '#fff', borderBottomWidth: 1, borderColor: '#E5E0D8' }}>
        ...
        <Text style={{ flex: 1, fontFamily: 'PlayfairDisplay-Bold', fontSize: 20, color: '#2C1810' }}>Modifier le plat</Text>
      ...
      <ScrollView style={{ flex: 1 }} contentContainerStyle={{ padding: 16, paddingBottom: 60 }} showsVerticalScrollIndicator={false}>
        ...
        {/* Photo area */}
        <TouchableOpacity style={{ height: 160, backgroundColor: '#F5F0EB', borderRadius: 18, borderWidth: 2, borderStyle: 'dashed', borderColor: '#E5E0D8', alignItems: 'center', justifyContent: 'center', marginBottom: 20 }}>
          <Icon name="Camera" size={32} color="rgba(140,130,120,0.4)" />
          <Text style={{ color: '#8C8278', fontSize: 14, marginTop: 8 }}>Ajouter une photo</Text>
        </TouchableOpacity>

        {/* Fields */}
        {fields.map((f, i) => (
          <View key={i} style={{ marginBottom: 16 }}>
            <Text style={{ fontSize: 11, fontWeight: '600', color: '#8C8278', textTransform: 'uppercase', letterSpacing: 0.8, marginBottom: 6 }}>{f.l}</Text>
            <View style={{ height: 48, borderWidth: 1, borderColor: '#E5E0D8', borderRadius: 16, backgroundColor: '#fff', paddingHorizontal: 14, justifyContent: 'center' }}>
              <TextInput
                ...
                style={{ fontSize: 14, color: '#2C1810' }} keyboardType={f.keyboard}
              />
            </View>
          </View>
        ))}
