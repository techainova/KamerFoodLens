...
const FOUND   = { name: 'Ndolé', region: 'Littoral', conf: 87 };
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: C.cream }}>
      ...
      <View style={{ height: 56, paddingHorizontal: 16, flexDirection: 'row', alignItems: 'center', gap: 10, backgroundColor: C.surface, borderBottomWidth: 1, borderColor: C.border }}>
        ...
        <Text style={{ flex: 1, fontFamily: 'PlayfairDisplay-Bold', fontSize: 18, color: C.ink }}>{t('scanner.result')}</Text>
      ...
      <ScrollView style={{ flex: 1 }} contentContainerStyle={{ padding: 16, paddingBottom: 40 }} showsVerticalScrollIndicator={false}>
        ...
        <View style={{ alignSelf: 'flex-start', flexDirection: 'row', alignItems: 'center', gap: 6, paddingHorizontal: 10, paddingVertical: 6, borderRadius: 20, backgroundColor: C.successSoft, marginBottom: 14 }}>
          ...
          <Text style={{ fontSize: 12, color: '#2E7D32', fontWeight: '600' }}>{t('scanner.confidence')} {FOUND.conf}%</Text>
        </View>

        {/* Title */}
        <Text style={{ fontSize: 36, fontFamily: 'PlayfairDisplay-Bold', color: C.ink, lineHeight: 42, marginBottom: 2 }}>{FOUND.name}</Text>
        <Text style={{ fontSize: 12, color: C.inkMute, marginBottom: 14 }}>{FOUND.region}</Text>
        ...
        <View style={{ flexDirection: 'row', gap: 8, marginBottom: 14 }}>
          {[
            { v: '90',     l: t('scanner.minutes') },
            { v: 'Mijoté', l: t('scanner.cookingDuration') },
            { v: 'Moyen',  l: t('scanner.spice') },
          ].map((s, i) => (
            <View key={i} style={{ flex: 1, borderWidth: 1, borderColor: C.border, borderRadius: 12, padding: 10, alignItems: 'center' }}>
              ...
              <Text style={{ fontSize: 11, color: C.inkMute }}>{s.l}</Text>
            </View>
          ))}
        </View>
        ...
        <View style={{ padding: 12, backgroundColor: C.surface2, borderRadius: 12, marginBottom: 14 }}>
          <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6 }}>
            <Text style={{ fontSize: 14, fontWeight: '700', color: C.ink }}>{t('scanner.storyTitle')}</Text>
            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 4 }}>
              <Icon name="Volume2" size={13} color="#E8591A" />
              <Text style={{ fontSize: 12, fontWeight: '600', color: '#E8591A' }}>{t('scanner.listenStory')}</Text>
            </View>
          </View>
          <Text style={{ fontSize: 12, color: C.inkSoft, lineHeight: 18 }}>
            ...
            <Text style={{ color: '#E8591A', fontWeight: '600' }}>{t('scanner.readMore')}</Text>
          </Text>
        </View>
        ...
        <View style={{ padding: 12, borderWidth: 1, borderColor: C.border, borderRadius: 12, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 }}>
          <Text style={{ fontSize: 13, fontWeight: '600', color: C.ink }}>{t('scanner.rateThisDish')}</Text>
        ...
        <View style={{ gap: 8 }}>
          <TouchableOpacity style={{ height: 56, backgroundColor: '#E8591A', borderRadius: 28, alignItems: 'center', justifyContent: 'center', flexDirection: 'row', gap: 8 }} activeOpacity={0.85}>
            <Text style={{ color: '#fff', fontSize: 16, fontWeight: '600' }}>{t('scanner.viewRecipe')}</Text>
          ...
          <TouchableOpacity style={{ height: 48, borderWidth: 2, borderColor: '#2E7D32', borderRadius: 24, alignItems: 'center', justifyContent: 'center' }}>
            <Text style={{ fontSize: 14, fontWeight: '600', color: '#2E7D32' }}>{t('scanner.findRestaurants')}</Text>
          </TouchableOpacity>
        </View>
      </ScrollView>
    </SafeAreaView>
