...
type InfoKey = 'history' | 'keyIngredients' | 'fullRecipe' | 'nearbyRestaurants' | 'nutritionInfo';

const INFO_DEFS: { key: InfoKey; icon: IconName }[] = [
  { key: 'history',           icon: 'BookOpen'   },
  { key: 'keyIngredients',    icon: 'List'       },
  { key: 'fullRecipe',        icon: 'FileText'   },
  { key: 'nearbyRestaurants', icon: 'MapPin'     },
  { key: 'nutritionInfo',     icon: 'BarChart2'  },
];
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: C.cream }}>
      ...
      <View style={{ height: 56, paddingHorizontal: 16, flexDirection: 'row', alignItems: 'center', gap: 10, backgroundColor: C.surface, borderBottomWidth: 1, borderColor: C.border }}>
        ...
        <Text style={{ flex: 1, fontFamily: 'PlayfairDisplay-Bold', fontSize: 18, color: C.ink }}>{t('scanner.analysis')}</Text>
      ...
      <ScrollView style={{ flex: 1 }} contentContainerStyle={{ padding: 16, paddingBottom: 40 }} showsVerticalScrollIndicator={false}>
        ...
        <View style={{ flexDirection: 'row', gap: 14, alignItems: 'center', marginBottom: 16 }}>
          ...
          <View style={{ flex: 1 }}>
            <Text style={{ fontSize: 11, color: C.inkMute, textTransform: 'uppercase', letterSpacing: 1 }}>{t('scanner.identified')}</Text>
            <Text style={{ fontSize: 22, fontFamily: 'PlayfairDisplay-Bold', color: C.ink, lineHeight: 28 }}>{FOUND.name}</Text>
        ...
        <View style={{ gap: 8, marginBottom: 18 }}>
          {INFO_DEFS.map((item, i) => (
            <TouchableOpacity key={i} style={{ flexDirection: 'row', gap: 12, alignItems: 'center', padding: 14, borderWidth: 1, borderColor: C.border, borderRadius: 18, backgroundColor: C.surface }}>
              ...
              <Text style={{ flex: 1, fontSize: 14, fontWeight: '600', color: C.ink }}>{t(`scanner.${item.key}`)}</Text>
        ...
        <View style={{ flexDirection: 'row', gap: 8 }}>
          <TouchableOpacity style={{ flex: 1, height: 48, backgroundColor: '#E8591A', borderRadius: 24, alignItems: 'center', justifyContent: 'center' }} activeOpacity={0.85}>
            <Text style={{ color: '#fff', fontSize: 14, fontWeight: '600' }}>{t('scanner.viewRecipe')}</Text>
          </TouchableOpacity>
          <TouchableOpacity style={{ flex: 1, height: 48, borderWidth: 2, borderColor: '#2E7D32', borderRadius: 24, alignItems: 'center', justifyContent: 'center' }}>
            <Text style={{ fontSize: 14, fontWeight: '600', color: '#2E7D32' }}>{t('common.share')}</Text>
          </TouchableOpacity>
        </View>
      </ScrollView>
    </SafeAreaView>
