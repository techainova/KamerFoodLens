  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: '#FFFAF5' }}>
      ...
      <View style={{ height: 56, paddingHorizontal: 16, flexDirection: 'row', alignItems: 'center', gap: 10, backgroundColor: '#fff', borderBottomWidth: 1, borderColor: '#E5E0D8' }}>
        ...
        <Text style={{ flex: 1, fontFamily: 'PlayfairDisplay-Bold', fontSize: 20, color: '#2C1810' }}>Paiement commande</Text>
      </View>
      ...
      <ScrollView style={{ flex: 1 }} contentContainerStyle={{ padding: 16, paddingBottom: 90 }} showsVerticalScrollIndicator={false}>
        ...
        <View style={{ alignItems: 'center', paddingVertical: 24, marginBottom: 16 }}>
          <Text style={{ fontSize: 11, color: '#8C8278', textTransform: 'uppercase', letterSpacing: 1.2, marginBottom: 4 }}>Montant à payer</Text>
          <Text style={{ fontSize: 36, fontFamily: 'PlayfairDisplay-Bold', color: '#E8591A' }}>{total.toLocaleString()} XAF</Text>
          <Text style={{ fontSize: 12, color: '#8C8278', marginTop: 4 }}>Commande #KFL-2026-4821</Text>
        </View>

        {/* Methods */}
        <Text style={{ fontSize: 15, fontFamily: 'PlayfairDisplay-Bold', color: '#2C1810', marginBottom: 12 }}>Choisissez votre moyen de paiement</Text>
        <View style={{ gap: 10, marginBottom: 16 }}>
          {METHODS.map(m => (
            <TouchableOpacity key={m.id} onPress={() => setMethod(m.id)}
              ...
              <View style={{ flex: 1 }}>
                <Text style={{ fontSize: 14, fontWeight: '600', color: '#2C1810' }}>{m.label}</Text>
                <Text style={{ fontSize: 12, color: '#8C8278' }}>{m.subLabel}</Text>
              </View>
        ...
        <View style={{ marginBottom: 16 }}>
          <Text style={{ fontSize: 11, fontWeight: '600', color: '#8C8278', textTransform: 'uppercase', letterSpacing: 0.8, marginBottom: 6 }}>Numéro de téléphone</Text>
          <View style={{ height: 48, borderWidth: 1, borderColor: '#E5E0D8', borderRadius: 16, backgroundColor: '#fff', paddingHorizontal: 14, flexDirection: 'row', alignItems: 'center', gap: 10 }}>
            ...
            <TextInput value={phone} onChangeText={setPhone} placeholder="+237 6X XX XX XX" placeholderTextColor="#8C8278" keyboardType="phone-pad" style={{ flex: 1, fontSize: 14, color: '#2C1810' }} />
          </View>
          <Text style={{ fontSize: 12, color: '#8C8278', marginTop: 4 }}>Vous recevrez une notification de paiement.</Text>
        </View>

        {/* Security note */}
        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
          <Icon name="Lock" size={14} color="#2E7D32" />
          <Text style={{ fontSize: 12, color: '#8C8278' }}>Paiement sécurisé AES-256-GCM</Text>
        </View>
      </ScrollView>
