  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: '#FFFAF5' }}>
      ...
      <View style={{ height: 56, paddingHorizontal: 16, flexDirection: 'row', alignItems: 'center', gap: 10, backgroundColor: '#fff', borderBottomWidth: 1, borderColor: '#E5E0D8' }}>
        ...
        <Text style={{ flex: 1, fontFamily: 'PlayfairDisplay-Bold', fontSize: 20, color: '#2C1810' }}>Moyens de paiement</Text>
      ...
      <ScrollView style={{ flex: 1 }} contentContainerStyle={{ padding: 16, paddingBottom: 60 }} showsVerticalScrollIndicator={false}>
        ...
        {/* Info banner */}
        <View style={{ padding: 16, borderRadius: 18, backgroundColor: '#FBF3DC', borderWidth: 1, borderColor: 'rgba(249,168,37,0.4)', marginBottom: 20, flexDirection: 'row', alignItems: 'flex-start', gap: 10 }}>
          <Icon name="Settings" size={16} color="#F9A825" />
          <Text style={{ flex: 1, fontSize: 14, color: '#2C1810' }}>Configurez les modes de paiement acceptés par votre restaurant.</Text>
        </View>

        <Text style={{ fontSize: 15, fontFamily: 'PlayfairDisplay-Bold', color: '#2C1810', marginBottom: 12 }}>Modes acceptés</Text>

        <View style={{ gap: 10, marginBottom: 20 }}>
          {METHODS.map((method) => (
            <View key={method.id} style={{ borderRadius: 18, backgroundColor: '#fff', borderWidth: 1, borderColor: '#E5E0D8', overflow: 'hidden', ...SHADOW_SM }}>
              <View style={{ flexDirection: 'row', alignItems: 'center', gap: 12, padding: 16 }}>
                ...
                <Text style={{ flex: 1, fontSize: 14, fontWeight: '600', color: '#2C1810' }}>{method.label}</Text>
                <Switch
              ...
              {enabled[method.id] && (method.id === 'mtn' || method.id === 'orange') && (
                <View style={{ paddingHorizontal: 16, paddingBottom: 16, borderTopWidth: 1, borderColor: '#F5F0EB' }}>
                  <Text style={{ fontSize: 11, color: '#8C8278', marginTop: 8, marginBottom: 6 }}>Numéro de compte</Text>
                  <View style={{ height: 40, backgroundColor: '#F5F0EB', borderWidth: 1, borderColor: '#E5E0D8', borderRadius: 12, paddingHorizontal: 12, justifyContent: 'center' }}>
                    <TextInput
        ...
        {/* Commission info */}
        <View style={{ padding: 16, borderRadius: 18, backgroundColor: '#E8EAF6', borderWidth: 1, borderColor: 'rgba(26,35,126,0.2)' }}>
          <Text style={{ fontSize: 14, fontWeight: '600', color: '#1A237E', marginBottom: 4 }}>Commission KFL</Text>
          <Text style={{ fontSize: 14, color: '#6D4C41', lineHeight: 20 }}>
