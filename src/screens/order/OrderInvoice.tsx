  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: C.cream }}>
      ...
      <View style={{ height: 56, paddingHorizontal: 16, flexDirection: 'row', alignItems: 'center', gap: 10, backgroundColor: C.surface, borderBottomWidth: 1, borderColor: C.border }}>
        ...
        <Text style={{ flex: 1, fontFamily: 'PlayfairDisplay-Bold', fontSize: 20, color: C.ink }}>Facture</Text>
      ...
      <ScrollView style={{ flex: 1 }} contentContainerStyle={{ padding: 16, paddingBottom: 80 }} showsVerticalScrollIndicator={false}>
        ...
        <View style={{ padding: 20, borderRadius: 20, backgroundColor: C.surface, borderWidth: 1, borderColor: C.border, marginBottom: 16, ...SHADOW_MD }}>
          <View style={{ alignItems: 'center', marginBottom: 16 }}>
            <View style={{ width: 48, height: 48, borderRadius: 24, backgroundColor: C.successSoft, borderWidth: 2, borderColor: '#2E7D32', alignItems: 'center', justifyContent: 'center', marginBottom: 8 }}>
              <Icon name="Check" size={22} color="#2E7D32" />
            </View>
            <Text style={{ fontSize: 18, fontFamily: 'PlayfairDisplay-Bold', color: C.ink }}>Payé</Text>
            <Text style={{ fontSize: 12, color: C.inkMute }}>15 Jun 2026 · 14:35</Text>
          </View>
        ...
        <View style={{ borderRadius: 18, backgroundColor: C.surface, borderWidth: 1, borderColor: C.border, marginBottom: 16 }}>
          {ORDER_ITEMS.map((item, i) => (
            <View key={i} style={{ flexDirection: 'row', justifyContent: 'space-between', paddingHorizontal: 16, paddingVertical: 12, borderBottomWidth: i < ORDER_ITEMS.length - 1 ? 1 : 0, borderColor: C.border }}>
              <Text style={{ fontSize: 14, color: C.ink }}>{item.name}</Text>
              <Text style={{ fontSize: 14, fontWeight: '600', color: C.ink }}>{item.price.toLocaleString()} XAF</Text>
            </View>
          ))}
          <View style={{ flexDirection: 'row', justifyContent: 'space-between', paddingHorizontal: 16, paddingVertical: 10, borderTopWidth: 1, borderColor: C.border }}>
            <Text style={{ fontSize: 14, color: C.inkMute }}>Livraison</Text>
            <Text style={{ fontSize: 14, color: C.inkSoft }}>{delivery.toLocaleString()} XAF</Text>
          </View>
        ...
        <View style={{ alignItems: 'center', paddingVertical: 20, marginBottom: 16 }}>
          <Text style={{ fontSize: 11, color: C.inkMute, textTransform: 'uppercase', letterSpacing: 1.2, marginBottom: 12 }}>QR Code · Facture</Text>
          ...
          <Text style={{ fontSize: 12, color: C.inkMute, marginTop: 8, fontFamily: 'JetBrainsMono-Regular' }}>#KFL-2026-4821</Text>
        </View>
      </ScrollView>
