  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: '#FFFAF5' }}>
      ...
      <View style={{ height: 56, paddingHorizontal: 16, flexDirection: 'row', alignItems: 'center', gap: 10, backgroundColor: '#fff', borderBottomWidth: 1, borderColor: '#E5E0D8' }}>
        ...
        <Text style={{ flex: 1, fontFamily: 'PlayfairDisplay-Bold', fontSize: 20, color: '#2C1810' }}>Statistiques</Text>
      </View>

      <ScrollView style={{ flex: 1 }} contentContainerStyle={{ padding: 16, paddingBottom: 40 }} showsVerticalScrollIndicator={false}>

        {/* Period selector */}
        <View style={{ flexDirection: 'row', backgroundColor: '#F5F0EB', borderRadius: 18, padding: 4, marginBottom: 20 }}>
          {PERIODS.map((p, i) => (
        ...
        <View style={{ flexDirection: 'row', gap: 10, marginBottom: 20 }}>
          {[
            ...
            <View key={i} style={{ flex: 1, padding: 14, borderRadius: 18, backgroundColor: '#fff', borderWidth: 1, borderColor: '#E5E0D8', alignItems: 'center', ...SHADOW_SM }}>
              <Text style={{ fontSize: 18, fontWeight: '700', color: s.color, fontFamily: 'PlayfairDisplay-Bold' }}>{s.v}</Text>
              <Text style={{ fontSize: 11, color: '#8C8278', textAlign: 'center', marginTop: 2 }}>{s.l}</Text>
            </View>
          ))}
        </View>

        {/* Calorie bar chart */}
        <Text style={{ fontSize: 15, fontFamily: 'PlayfairDisplay-Bold', color: '#2C1810', marginBottom: 12 }}>Calories par jour</Text>
        <View style={{ padding: 16, borderRadius: 18, backgroundColor: '#fff', borderWidth: 1, borderColor: '#E5E0D8', marginBottom: 20, ...SHADOW_SM }}>
          ...
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8, marginBottom: 8 }}>
            <View style={{ width: 20, height: 1, borderWidth: 1, borderStyle: 'dashed', borderColor: '#E5E0D8' }} />
            <Text style={{ fontSize: 11, color: '#8C8278' }}>Objectif : 2 000 kcal</Text>
          </View>
          <View style={{ flexDirection: 'row', justifyContent: 'space-around' }}>
            {WEEK_DATA.map((d, i) => (
              <Text key={i} style={{ fontSize: 11, color: '#8C8278', width: 28, textAlign: 'center' }}>{d.day}</Text>
            ))}
          </View>
        </View>

        {/* Top dishes */}
        <Text style={{ fontSize: 15, fontFamily: 'PlayfairDisplay-Bold', color: '#2C1810', marginBottom: 12 }}>Plats les plus consommés</Text>
        <View style={{ gap: 10, marginBottom: 20 }}>
          {TOP_DISHES.map((dish, i) => (
            <View key={i} style={{ flexDirection: 'row', alignItems: 'center', gap: 12, padding: 14, borderRadius: 18, backgroundColor: '#fff', borderWidth: 1, borderColor: '#E5E0D8', ...SHADOW_SM }}>
              ...
              <Text style={{ flex: 1, fontSize: 14, fontWeight: '600', color: '#2C1810' }}>{dish.name}</Text>
        ...
        {/* Nutrients */}
        <Text style={{ fontSize: 15, fontFamily: 'PlayfairDisplay-Bold', color: '#2C1810', marginBottom: 12 }}>Macronutriments moyens</Text>
        <View style={{ padding: 16, borderRadius: 18, backgroundColor: '#fff', borderWidth: 1, borderColor: '#E5E0D8', ...SHADOW_SM }}>
          {[
            ...
            <View key={i} style={{ paddingVertical: 10, borderBottomWidth: i < 3 ? 1 : 0, borderColor: '#F5F0EB' }}>
              <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginBottom: 4 }}>
                <Text style={{ fontSize: 14, color: '#2C1810' }}>{n.l}</Text>
                <Text style={{ fontSize: 12, color: '#8C8278' }}>{n.v}</Text>
              </View>
